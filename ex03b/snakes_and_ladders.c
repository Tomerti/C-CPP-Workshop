#include <string.h> // For strlen(), strcmp(), strcpy()
#include "markov_chain.h"

#define MAX(X, Y) (((X) < (Y)) ? (Y) : (X))

#define EMPTY -1
#define BOARD_SIZE 100
#define MAX_GENERATION_LENGTH 60

#define DICE_MAX 6
#define NUM_OF_TRANSITIONS 20
#define INC "Incorrect number of args, should insert 3"
#define THREE 3
#define TEN 10


/**
 * represents the transitions by ladders and snakes in the game
 * each tuple (x,y) represents a ladder from x to if x<y or a snake otherwise
 */
const int transitions[][2] = {{13, 4},
                              {85, 17},
                              {95, 67},
                              {97, 58},
                              {66, 89},
                              {87, 31},
                              {57, 83},
                              {91, 25},
                              {28, 50},
                              {35, 11},
                              {8,  30},
                              {41, 62},
                              {81, 43},
                              {69, 32},
                              {20, 39},
                              {33, 70},
                              {79, 99},
                              {23, 76},
                              {15, 47},
                              {61, 14}};

void create_markov(MarkovChain *markov_chain,LinkedList *database);
static void print_func (void *data);
static int compare_func (void *data1, void *data2);
static void free_data (void *data);
static void *copy_func (void *data);
static bool is_last (void *data);

/**
 * struct represents a Cell in the game board
 */
typedef struct Cell {
    int number; // Cell number 1-100
    int ladder_to;  // ladder_to represents the jump of the ladder in case
    // there is one from this square
    int snake_to;  // snake_to represents the jump of the snake in case there
    // is one from this square
    //both ladder_to and snake_to should be -1 if the Cell doesn't have them
} Cell;

/** Error handler **/
static int handle_error(char *error_msg, MarkovChain **database)
{
    printf("%s", error_msg);
    if (database != NULL)
    {
        free_database(database);
    }
    return EXIT_FAILURE;
}


static int create_board(Cell *cells[BOARD_SIZE])
{
    for (int i = 0; i < BOARD_SIZE; i++)
    {
        cells[i] = malloc(sizeof(Cell));
        if (cells[i] == NULL)
        {
            for (int j = 0; j < i; j++) {
                free(cells[j]);
            }
            handle_error(ALL_ERROR,NULL);
            return EXIT_FAILURE;
        }
        *(cells[i]) = (Cell) {i + 1, EMPTY, EMPTY};
    }

    for (int i = 0; i < NUM_OF_TRANSITIONS; i++)
    {
        int from = transitions[i][0];
        int to = transitions[i][1];
        if (from < to)
        {
            cells[from - 1]->ladder_to = to;
        }
        else
        {
            cells[from - 1]->snake_to = to;
        }
    }
    return EXIT_SUCCESS;
}

/**
 * fills database
 * @param markov_chain
 * @return EXIT_SUCCESS or EXIT_FAILURE
 */
static int fill_database(MarkovChain *markov_chain)
{
    Cell* cells[BOARD_SIZE];
    if(create_board(cells) == EXIT_FAILURE)
    {
        return EXIT_FAILURE;
    }
    MarkovNode *from_node = NULL, *to_node = NULL;
    size_t index_to;
    for (size_t i = 0; i < BOARD_SIZE; i++)
    {
        add_to_database(markov_chain, cells[i]);
    }

    for (size_t i = 0; i < BOARD_SIZE; i++)
    {
        from_node = get_node_from_database(markov_chain,cells[i])->data;

        if (cells[i]->snake_to != EMPTY || cells[i]->ladder_to != EMPTY)
        {
            index_to = MAX(cells[i]->snake_to,cells[i]->ladder_to) - 1;
            to_node = get_node_from_database(markov_chain, cells[index_to])
                    ->data;
            add_node_to_frequencies_list (from_node, to_node, markov_chain);
        }
        else
        {
            for (int j = 1; j <= DICE_MAX; j++)
            {
                index_to = ((Cell*) (from_node->data))->number + j - 1;
                if (index_to >= BOARD_SIZE)
                {
                    break;
                }
                to_node = get_node_from_database(markov_chain,
                                                 cells[index_to])
                        ->data;
                add_node_to_frequencies_list
                (from_node, to_node, markov_chain);
            }
        }
    }
    // free temp arr
    for (size_t i = 0; i < BOARD_SIZE; i++)
    {
        free(cells[i]);
    }
    return EXIT_SUCCESS;
}

/**
 * @param argc num of arguments
 * @param argv 1) Seed
 *             2) Number of sentences to generate
 * @return EXIT_SUCCESS or EXIT_FAILURE
 */

int get_args(int argc, char* argv[],unsigned int* seed,unsigned int* routes){
    if(argc!=THREE){
        printf(INC);
        return EXIT_FAILURE;
    }
    char* p;
    *seed = (unsigned int)strtol(argv[1], &p, TEN);
    *routes = (unsigned int) strtol(argv[2], &p, TEN);
    return EXIT_SUCCESS;
}

static void print_func (void *data)
{
    Cell *check = (Cell *) data;
    if(check->ladder_to == EMPTY && check->snake_to == EMPTY){
        if(is_last(data)){
            printf("[%d]",check->number);
        }
        else{
            printf("[%d] -> ",check->number);
        }
    }
    else if(check->ladder_to != EMPTY && check->snake_to == EMPTY){
        printf("[%d]-ladder to %d -> ",check->number,check->ladder_to);
    }
    else if(check->ladder_to == EMPTY && check->snake_to != EMPTY){
        printf("[%d]-snake to %d -> ",check->number,check->snake_to);
    }
}

static int compare_func (void *data1, void *data2)
{
    Cell* check1 = (Cell *) data1;
    Cell* check2 = (Cell *) data2;
    if(check1->number > check2->number){
        return 1;
    }
    if(check1->number < check2->number){
        return -1;
    }
    return 0;
}

static void free_data (void *data)
{
    Cell* check = (Cell*) data;
    free(check);
}

static void *copy_func (void *data)
{
    Cell *check = (Cell *) data;
    Cell *copy = malloc(sizeof(Cell));
    if(copy == NULL){
        return NULL;
    }
    copy->number = check->number;
    copy->ladder_to = check->ladder_to;
    copy->snake_to = check->snake_to;
    return copy;
}

static bool is_last (void *data)
{
    Cell *check = (Cell *) data;
    if(check->number == BOARD_SIZE){
        return true;
    }
    return false;
}

int main(int argc, char *argv[])
{
    unsigned int seed;
    unsigned int routes;
    if(get_args(argc, argv, &seed,&routes)){
        return EXIT_FAILURE;
    }
    srand(seed);
    LinkedList* database = malloc (sizeof (LinkedList));
    if(database == NULL){
        printf (ALL_ERROR);
        return EXIT_FAILURE;
    }
    MarkovChain* markov_chain = malloc (sizeof (MarkovChain));
    if(markov_chain == NULL){
        free(database);
        database = NULL;
        printf (ALL_ERROR);
        return EXIT_FAILURE;
    }
    create_markov(markov_chain, database);
    if(fill_database(markov_chain)==1){
        printf (ALL_ERROR);
        free_database(&markov_chain);
        free(database);
        database = NULL;
        markov_chain = NULL;
        return EXIT_FAILURE;
    }
    MarkovNode* first = markov_chain->database->first->data;
    for(unsigned int i=1;i<=routes;i++){
        printf("Random Walk %d: ",i);
        generate_tweet(markov_chain,first,MAX_GENERATION_LENGTH);
    }
    free_database(&markov_chain);
    return EXIT_SUCCESS;
}

void create_markov(MarkovChain *markov_chain,LinkedList *database)
{
    markov_chain->database = database;
    markov_chain->database->first = NULL;
    markov_chain->database->last = NULL;
    markov_chain->database->size = 0;
    markov_chain->free_data = &free_data;
    markov_chain->copy_func = &copy_func;
    markov_chain->comp_func = &compare_func;
    markov_chain->is_last = &is_last;
    markov_chain->print_func = &print_func;
}